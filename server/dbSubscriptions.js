...
Meteor.publish('allUsers', function (filterObj, authObj) {
  ...
  let query = `select ru.ru_status, ru.ru_application_survey, u.*, 
  ...
  //debug                
  //logger.log('Query with filters : ', query);

  var res = liveDb.select(query, [bootcampId]);
  return res;
});
...
Meteor.publish('bootcampClasses', function (authObj) {
  ...
  var res = liveDb.select('select distinct b.*, r.r_id, r.r_min_students,r.r_max_students, r.r_start_date, r.r_b_id, \
                          r.r_usd_price, r.r_closed_dates, r.r_start_date, r.r_start_time_mins \
                              from v5_bootcamps b \
                            left join v5_classes r on b.b_id = r.r_b_id and r.r_status >= 1\
                            where \
                            b.b_id = $1 ', [bootcampId]);
  return res;
